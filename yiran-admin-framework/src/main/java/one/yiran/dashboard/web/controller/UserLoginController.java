...
//            throw new UserNotFoundException(username);
        ...
        SysChannel channel = sysChannelService.selectByPId(user.getChannelId());
        if(channel == null) {
            if (!user.isAdmin()){
                throw BusinessException.build("用户渠道不存在");
            }
        } else {
            if (!user.isAdmin() && channel.getExpireDate().isBefore(LocalDate.now())) {
                throw BusinessException.build("商户已过期，请联系管理员");
            }
            us.setChannelCode(channel.getChannelCode());
            us.setChannelName(channel.getChannelName());
            randomKey = channel.getChannelCode() + "_" + user.getUserId() + "_" + RandomStringUtils.randomAlphanumeric(38);
        }

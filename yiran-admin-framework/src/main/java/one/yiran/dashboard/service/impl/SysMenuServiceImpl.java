...
public class SysMenuServiceImpl extends CrudBaseServiceImpl<Long, SysMenu> implements SysMenuService {
    ...
    public long countMenuByParentId(Long parentId) {
        ...
        return this.count(qSysMenu.parentId.eq(parentId)
                .and(qSysMenu.isDelete.isNull().or(qSysMenu.isDelete.eq(Boolean.FALSE))));
    }

    private List<SysMenu> findAllByParentId(Long parentId) {
        QSysMenu qSysMenu = QSysMenu.sysMenu;
        return this.selectList(qSysMenu.parentId.eq(parentId)
                .and(qSysMenu.isDelete.isNull().or(qSysMenu.isDelete.eq(Boolean.FALSE))));
    }

    private SysMenu findByMenuName(String menuName) {
        QSysMenu qSysMenu = QSysMenu.sysMenu;
        return this.selectOne(qSysMenu.menuName.eq(menuName)
                .and(qSysMenu.isDelete.isNull().or(qSysMenu.isDelete.eq(Boolean.FALSE))));
    }
...
//            }
        ...
        if (m.getMenuId() != null) {
            List<SysMenu> childs = this.findAllByParentId(m.getMenuId());
    ...
    public boolean checkMenuNameUnique(SysMenu sysMenu) {
        ...
        SysMenu m = this.findByMenuName(sysMenu.getMenuName());

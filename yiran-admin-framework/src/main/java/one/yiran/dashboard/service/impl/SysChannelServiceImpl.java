...
import one.yiran.dashboard.event.ChannelEvent;
...
public class SysChannelServiceImpl extends CrudBaseServiceImpl<Long, SysChannel> implements SysChannelService {
    ...
    @Override
    public SysChannel selectByH5DomainName(String domainName) {
        Assert.hasLength(domainName,"domainName不能为空");
        return selectOne(QSysChannel.sysChannel.h5domainName.eq(domainName));
    }
    ...
    public SysChannel create(SysChannel channel) {
        ...
        applicationEventPublisher.publishEvent(new ChannelEvent(this,db));
        return db;
    }
}

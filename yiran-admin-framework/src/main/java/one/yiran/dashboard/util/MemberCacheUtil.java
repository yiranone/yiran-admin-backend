...
public class MemberCacheUtil {
    ...
    public static void setSessionInfo(String key, MemberSession member) {
        //踢掉上一个
        Set<String> keys = getCacheService().keys(Global.getRedisPrefix() + SESSION_PREFIX + member.getChannelCode() + "_" + member.getMemberId() + "_*");
    ...
    public static void removeAllSessionInfo(String channelCode, Long memberId) {
        Set<String> keys = getCacheService().keys(Global.getRedisPrefix() + SESSION_PREFIX + channelCode + "_" + memberId + "_*");
        if (null != keys && !keys.isEmpty()) {
            for (String k : keys) {
                log.info("清理用户{}登录的信息:{}", memberId, k);
                clearKey(k);
            }
        }
    }

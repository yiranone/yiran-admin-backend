...
public class MemberInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if((session = MemberCacheUtil.getSessionInfo(token)) == null) {
            ...
            throw new UserNotLoginException("The user is not logged in, or the login has expired");
        } else if (session.getIsLocked() == Boolean.TRUE) {
            throw BusinessException.build("Your account is frozen, please contact customer service");
        }

        request.setAttribute(ServletRequestUtil.HTTP_SERVLET_KEY_LOGIN_ID, session.getMemberId());
        request.setAttribute(ServletRequestUtil.HTTP_SERVLET_KEY_LOGIN_TYPE, UserConstants.SESSION_TYPE_MEMBER);
        request.setAttribute(ServletRequestUtil.HTTP_SERVLET_KEY_LOGIN_NAME, session.getLoginAddress());
        return true;
    }

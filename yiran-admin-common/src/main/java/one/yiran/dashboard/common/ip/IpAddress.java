...
public class IpAddress {
	...
	public static int compareUnsigned(byte[] a, byte[] b) {
		...
		for (int i = 0; i < minLength; i++) {
			// 将字节转换为无符号整数进行比较
			int unsignedA = Byte.toUnsignedInt(a[i]);
			int unsignedB = Byte.toUnsignedInt(b[i]);
			int result = Integer.compareUnsigned(unsignedA, unsignedB);
			if (result != 0) {
				return result;
			}
		}
	...
	public int compareTo(IpAddress anIp) {
		return compareUnsigned(m_ip, anIp.getBytes());
	}

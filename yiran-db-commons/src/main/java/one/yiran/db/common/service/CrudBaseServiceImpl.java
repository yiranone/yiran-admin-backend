...
public class CrudBaseServiceImpl<K,T> implements CrudBaseService<K,T> {
    ...
    public T selectByPId(K pId) {
        ...
        if(rs == null || rs.size() == 0){
            ...
            throw BusinessException.build("selectByPId query multiple results");
        }
        return rs.get(0);
    }
    ...
    public T selectOne(Predicate predicate) {
        ...
        if(rs == null || rs.size() == 0){
            ...
            throw BusinessException.build("selectOne queries multiple results");
        }
        return rs.get(0);
    }
...
//        return deletedCount;
    ...
    public long delete(Predicate predicate) throws BusinessException {
        ...
        if(predicate == null)
            throw BusinessException.build("predicate cannot be empty");
